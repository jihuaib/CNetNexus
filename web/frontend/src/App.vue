...
async function hydrateFromSnapshot(snap, { reason = '导入', restoreRunning = false } = {})
{
    ...
    try
    {
        ...
        {
            const data = await r.json();
            const running = new Map((data.instances || []).map(i => [i.id, i]));
            for (const n of nodes)
            {
                const inst = running.get(n.id);
                if (inst)
                {
                    n.instance = inst;
                    n.status = 'running';
    ...
    // 恢复模式：对每个带 db 的 node，把现有容器删掉后再用 pendingDb 重启，保证配置完整落回去。
    // 仅在 "从文件导入" 时跑；页面刷新后从 localStorage 自动恢复不走这里。
    if (restoreRunning)
    {
        const withDb = nodes.filter(n => n.pendingDb);
        if (withDb.length > 0)
        {
            pushLog(`开始用导出的 db 恢复 ${withDb.length} 个设备 ...`);
...
async function stopNode(node)
{
    ...
    try
    {
        await fetch(`/api/instances/${node.id}`, { method: 'DELETE' });
        node.status = 'stopped';
        node.instance = null;
        pushLog(`${node.label} 已停止`);
