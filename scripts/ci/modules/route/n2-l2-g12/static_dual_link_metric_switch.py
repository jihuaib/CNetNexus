#!/usr/bin/env python3
"""
Route static best-path switch check on dual links, dual-stack.

Goal:
- install two static IPv4/IPv6 routes for one prefix via GE-1/GE-2 peer gateways
...
V6_TARGET_PREFIX_ADDR = "2001:db8:203:113::"
...
def _wait_route_paths_ipv6(
    ...
    *,
    ...
    wait_check(
        ...
        interval=interval,
        regex=path_regex,
        label=f"{device} ipv6 route-paths {destination}",
    )


def _wait_connected_os_if_ipv6(
    rt: TopologyRuntime,
    *,
    device: str,
    prefix: str,
...
def _wait_os_best_ipv6(
    ...
    *,
    ...
    wait_check(
        ...
        regex=[row_regex],
        not_regex=stale_regex,
        label=f"{device} ipv6 os-best {prefix} via {expect_gateway} dev {expect_interface}",
    )
    prefix_addr, prefix_len = prefix.rsplit("/", 1)
    wait_fib_ipv6_route(
        rt,
...
def _cleanup_ipv6(rt: TopologyRuntime, *, device: str, nh_ge1: str, nh_ge2: str) -> None:
    run_cmds(
        rt=rt,
        device=device,
        strict=False,
...
def run(rt: TopologyRuntime, top: dict[str, object]) -> None:
    _run_ipv4(rt, top)
    _run_ipv6(rt, top)
    print("Route dual-link static metric switch dual-stack check passed.")
...
def _run_ipv6(rt: TopologyRuntime, top: dict[str, object]) -> None:
    ...
    try:
        ...
        _cleanup_ipv6(rt, device="r1", nh_ge1=ge1_nh, nh_ge2=ge2_nh)
        ...
        ge1_os_if = _wait_connected_os_if_ipv6(rt, device="r1", prefix=ge1_connected_prefix, timeout=20)
        ge2_os_if = _wait_connected_os_if_ipv6(rt, device="r1", prefix=ge2_connected_prefix, timeout=20)
        print(f"Detected OS interfaces: {ge1_name}->{ge1_os_if}, {ge2_name}->{ge2_os_if}")
        ...
        step("Raise GE-1 IPv6 metric and verify GE-2 takeover")
        run_cmds(
            rt=rt,
            device="r1",
            commands=[
        ...
        step("Lower GE-1 IPv6 metric and verify switch back")
        run_cmds(
            rt=rt,
            device="r1",
            commands=[
