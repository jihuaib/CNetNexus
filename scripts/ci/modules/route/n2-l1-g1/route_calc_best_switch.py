#!/usr/bin/env python3
"""
Route calc best-path switch check (dual-stack static metric based).

Goal:
- install two static IPv4/IPv6 paths for the same prefix (both reachable)
...
from module_api import cmd, g_top, require_devices, run_cmds, step, wait_check, wait_checks, wait_fib_ipv4_route, wait_fib_ipv6_route  # noqa: E402
...
GE_IF = "GE-1"
V6_TARGET_PREFIX_ADDR = "2001:db8:203:100::"
V6_TARGET_PREFIX_LEN = 64
V6_TARGET_PREFIX = f"{V6_TARGET_PREFIX_ADDR}/{V6_TARGET_PREFIX_LEN}"
V6_RESOLVER_ADDR = "2001:db8:198:51::1"
V6_RESOLVER_PREFIX_LEN = 128
...
def _cleanup(
    ...
    *,
    ...
    run_cmds(
        ...
        commands=[
            "config",
            f"no route static ipv4 {TARGET_PREFIX_ADDR} {TARGET_MASK} {primary_nh}",
            f"no route static ipv4 {TARGET_PREFIX_ADDR} {TARGET_MASK} {secondary_nh}",
            f"no route static ipv4 {RESOLVER_ADDR} {RESOLVER_MASK} {primary_nh}",
            "end",
        ],
    )


def _wait_path_total_ipv6(
    rt: TopologyRuntime,
    *,
    device: str,
    destination: str,
    ...
    wait_check(
        rt,
        device=device,
        command=f"show route ipv6 {destination} {V6_TARGET_PREFIX_LEN}",
        timeout=timeout,
        interval=interval,
        contains=[f"Total {expect_total} path(s)"],
        label=f"{device} ipv6 path-total {destination}={expect_total}",
    )


def _wait_route_metrics_ipv6(
    rt: TopologyRuntime,
    *,
    device: str,
    destination: str,
    ...
    wait_check(
        ...
        regex=metric_regex,
        not_contains=[f"Nexthop : {nh}" for nh in absent_nhs],
        label=f"{device} ipv6 route-metrics {destination}",
    )


def _wait_first_path_os_installed_flag_ipv6(
    rt: TopologyRuntime,
    *,
    device: str,
    destination: str,
    ...
    last_out = ""
    while time.time() < deadline:
        out = cmd(rt, device, f"show route ipv6 {destination} {V6_TARGET_PREFIX_LEN}", strict=False)
        last_out = out
        if _path1_flag_set(out):
            return
        time.sleep(interval)
    ...
    raise RuntimeError(
        f"{device} path[1] os-installed flag check timeout after {timeout}s\n"
        f"expect: Path [1] contains Flags with bit0 set\n"
        f"command: show route ipv6 {destination} {V6_TARGET_PREFIX_LEN}\n"
        f"last output:\n{last_out}"
    )


def _wait_os_main_gateway_ipv6(
    rt: TopologyRuntime,
    *,
    device: str,
    prefix: str,
    ...
    wait_check(
        ...
        label=f"{device} ipv6 os-best {prefix} via {expect_gateway} metric={expect_metric if expect_metric is not None else '*'}",
    )
    prefix_addr, prefix_len = prefix.rsplit("/", 1)
    wait_fib_ipv6_route(
        rt,
        ...
        timeout=timeout,
        interval=interval,
        label=f"{device} ipv6 fib-best {prefix} via {expect_gateway}",
    )
...
def _cleanup_ipv6(
    ...
    *,
    ...
    run_cmds(
        ...
        commands=[
            "config",
            f"no route static ipv6 {V6_TARGET_PREFIX_ADDR} {V6_TARGET_PREFIX_LEN} {primary_nh}",
            f"no route static ipv6 {V6_TARGET_PREFIX_ADDR} {V6_TARGET_PREFIX_LEN} {secondary_nh}",
            f"no route static ipv6 {V6_RESOLVER_ADDR} {V6_RESOLVER_PREFIX_LEN} {primary_nh}",
            f"if {GE_IF}",
            "no shutdown",
            "exit",
            "end",
...
def run(rt: TopologyRuntime, top: dict[str, object]) -> None:
    _run_ipv4(rt, top)
    _run_ipv6(rt, top)
    print("Route calc best-path switch dual-stack check passed.")
...
def _run_ipv4(rt: TopologyRuntime, top: dict[str, object]) -> None:
    ...
    try:
        ...
        run_cmds(
            ...
            commands=[
                "config",
                f"route static ipv4 {RESOLVER_ADDR} {RESOLVER_MASK} {primary_nh}",
                "end",
            ],
        )
        ...
        run_cmds(
            ...
            commands=[
                "config",
                f"route static ipv4 {TARGET_PREFIX_ADDR} {TARGET_MASK} {primary_nh} metric 10",
                f"route static ipv4 {TARGET_PREFIX_ADDR} {TARGET_MASK} {secondary_nh} metric 20",
                "end",
            ],
        )
        ...
        run_cmds(
            ...
            commands=[
                "config",
                f"route static ipv4 {TARGET_PREFIX_ADDR} {TARGET_MASK} {primary_nh} metric 30",
                "end",
            ],
        )
        _wait_path_total(rt, device="r1", destination=TARGET_PREFIX_ADDR, expect_total=2, timeout=30)
        ...
        run_cmds(
            ...
            commands=[
                "config",
                f"route static ipv4 {TARGET_PREFIX_ADDR} {TARGET_MASK} {primary_nh} metric 5",
                "end",
            ],
        )
        _wait_path_total(rt, device="r1", destination=TARGET_PREFIX_ADDR, expect_total=2, timeout=30)
        ...
        run_cmds(
            ...
            commands=[
                "config",
                f"route static ipv4 {TARGET_PREFIX_ADDR} {TARGET_MASK} {primary_nh} metric 1",
                "end",
            ],
        )
        _wait_path_total(rt, device="r1", destination=TARGET_PREFIX_ADDR, expect_total=2, timeout=30)
...
def _run_ipv6(rt: TopologyRuntime, top: dict[str, object]) -> None:
    ...
    try:
        ...
        step("Add IPv6 resolver route for secondary nexthop")
        run_cmds(
            rt=rt,
            device="r1",
            commands=[
                "config",
                f"route static ipv6 {V6_RESOLVER_ADDR} {V6_RESOLVER_PREFIX_LEN} {primary_nh}",
                "end",
            ],
        )

        step("Add two static IPv6 candidate paths for same prefix")
        run_cmds(
            rt=rt,
            device="r1",
            commands=[
                "config",
                f"route static ipv6 {V6_TARGET_PREFIX_ADDR} {V6_TARGET_PREFIX_LEN} {primary_nh} metric 10",
                f"route static ipv6 {V6_TARGET_PREFIX_ADDR} {V6_TARGET_PREFIX_LEN} {secondary_nh} metric 20",
                "end",
            ],
        )

        step("Verify lower metric IPv6 path selected as OS best")
        _wait_path_total_ipv6(rt, device="r1", destination=V6_TARGET_PREFIX_ADDR, expect_total=2, timeout=30)
        _wait_route_metrics_ipv6(
            rt,
            device="r1",
            destination=V6_TARGET_PREFIX_ADDR,
            expect_metrics={
                primary_nh: 10,
                secondary_nh: 20,
            },
        ...
        _wait_first_path_os_installed_flag_ipv6(rt, device="r1", destination=V6_TARGET_PREFIX_ADDR, timeout=30)
        _wait_os_main_gateway_ipv6(
            rt,
            device="r1",
            prefix=V6_TARGET_PREFIX,
            expect_gateway=primary_nh,
            expect_metric=None,
            timeout=30,
        )

        step("Raise primary IPv6 metric and verify route metric update")
        run_cmds(
            rt=rt,
            device="r1",
            commands=[
                "config",
                f"route static ipv6 {V6_TARGET_PREFIX_ADDR} {V6_TARGET_PREFIX_LEN} {primary_nh} metric 30",
                "end",
            ],
        )
        _wait_path_total_ipv6(rt, device="r1", destination=V6_TARGET_PREFIX_ADDR, expect_total=2, timeout=30)
        ...
        step("Lower primary IPv6 metric and verify route metric update")
        run_cmds(
            rt=rt,
            device="r1",
            commands=[
                "config",
                f"route static ipv6 {V6_TARGET_PREFIX_ADDR} {V6_TARGET_PREFIX_LEN} {primary_nh} metric 5",
                "end",
            ],
        )
        _wait_path_total_ipv6(rt, device="r1", destination=V6_TARGET_PREFIX_ADDR, expect_total=2, timeout=30)
        ...
        step("Withdraw primary IPv6 and verify secondary takeover")
        run_cmds(
            rt=rt,
            device="r1",
            commands=[
        ...
        _wait_path_total_ipv6(rt, device="r1", destination=V6_TARGET_PREFIX_ADDR, expect_total=1, timeout=30)
        _wait_route_metrics_ipv6(
            rt,
            device="r1",
            destination=V6_TARGET_PREFIX_ADDR,
        ...
        step("Re-add primary IPv6 and verify preemption")
        run_cmds(
            rt=rt,
            device="r1",
            commands=[
                "config",
                f"route static ipv6 {V6_TARGET_PREFIX_ADDR} {V6_TARGET_PREFIX_LEN} {primary_nh} metric 1",
                "end",
            ],
        )
        _wait_path_total_ipv6(rt, device="r1", destination=V6_TARGET_PREFIX_ADDR, expect_total=2, timeout=30)
