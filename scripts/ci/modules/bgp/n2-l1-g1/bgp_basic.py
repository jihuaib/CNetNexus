...
def _cleanup_case_config(rt: TopologyRuntime, r1_route_nh: str) -> None:
    ...
    run_cmds(
        ...
        commands=[
            "config",
            f"no route ipv4 10.10.10.0 24 {r1_route_nh}",
            "no bgp",
            "end",
        ],
    )
...
def run(rt: TopologyRuntime, top: dict[str, object]) -> None:
    ...
    try:
        ...
        run_cmds(
            rt=rt,
            device="r1",
            commands=["config", f"route ipv4 10.10.10.0 24 {r1_peer_ip}", "end"],
        )
        ...
        _cleanup_case_config(rt, r1_peer_ip)
