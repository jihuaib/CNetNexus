...
def _cleanup_case_config(rt: TopologyRuntime, r1_peer_ip6: str) -> None:
    step("Cleanup IPv6 BGP/static config")
    run_cmds(
        rt=rt,
        device="r1",
        strict=False,
...
def run(rt: TopologyRuntime, top: dict[str, object]) -> None:
    ...
    try:
        ...
        step("Configure IPv6 BGP neighbors")
        run_cmds(
            rt=rt,
            device="r1",
            commands=[
        ...
        step("Wait IPv6 BGP sessions")
        wait_checks(rt, session_checks, timeout=40)

        step("Apply static IPv6 route on r1")
        run_cmds(
            rt=rt,
            device="r1",
            commands=["config", f"route ipv6 {TEST_V6_PREFIX} {TEST_V6_PREFIX_LEN} {r1_peer_ip6}", "end"],
        )
        ...
        step("Wait IPv6 BGP routes")
        wait_checks(rt, route_checks, timeout=40)

        step("Reboot r1 and wait CLI reconnect")
        reboot_device(rt, "r1", timeout=90)

        step("Wait IPv6 BGP sessions after reboot")
        wait_checks(rt, session_checks, timeout=40)

        step("Wait IPv6 BGP routes after reboot")
        wait_checks(rt, route_checks, timeout=40)

        print("BGP basic IPv6 check passed.")
    finally:
        _cleanup_case_config(rt, r1_peer_ip6)
