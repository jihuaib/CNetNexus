#!/usr/bin/env python3
"""
BGP loopback-peer IPv6 check script.
...
R1_LOOP_ID = 201
R2_LOOP_ID = 202
R1_LOOP_IP = "2001:db8:201::1"
R2_LOOP_IP = "2001:db8:202::1"
HOST_MASK = "128"
...
def _cleanup_case_config(rt: TopologyRuntime, *, r1_peer_ip6: str, r2_peer_ip6: str) -> None:
    ...
    run_cmds(
        ...
        commands=[
            "config",
            f"no route ipv6 {R2_LOOP_IP} {HOST_MASK} {r1_peer_ip6}",
    ...
    run_cmds(
        ...
        commands=[
            "config",
            f"no route ipv6 {R1_LOOP_IP} {HOST_MASK} {r2_peer_ip6}",
...
def _assert_not_established(rt: TopologyRuntime, *, device: str, peer_ip: str, timeout: int, interval: int = 2) -> None:
    hold_check(
        ...
        duration=timeout,
        interval=interval,
        not_regex=[rf"(?im)^.*{re.escape(peer_ip)}.*\bEstablished\b.*$"],
    ...
    try:
        ...
        run_cmds(
            ...
            commands=[
                ...
                f"ipv6 address {R1_LOOP_IP} {HOST_MASK}",
                "exit",
                "end",
            ],
        )
        run_cmds(
            ...
            commands=[
                ...
                f"ipv6 address {R2_LOOP_IP} {HOST_MASK}",
                "exit",
                "end",
            ],
        )
            ...
            [
                {
                    ...
                    "contains": [
                        ...
                        "State      : UP",
                        f"IPv6 Addr  : {R1_LOOP_IP}/{HOST_MASK}",
                    ],
                ...
                },
                {
                    ...
                    "contains": [
                        ...
                        "State      : UP",
                        f"IPv6 Addr  : {R2_LOOP_IP}/{HOST_MASK}",
                    ],
        ...
        run_cmds(
            ...
            commands=[
                "config",
                f"route ipv6 {R2_LOOP_IP} {HOST_MASK} {r1_peer_ip6}",
                "end",
            ],
        )
        run_cmds(
            ...
            commands=[
                "config",
                f"route ipv6 {R1_LOOP_IP} {HOST_MASK} {r2_peer_ip6}",
                "end",
            ],
        )
        wait_checks(
            ...
            [
                {
                    ...
                    "contains": [f"{R2_LOOP_IP}/{HOST_MASK}", r1_peer_ip6],
                    "label": "r1 static route to r2 loopback",
                },
                {
                    "device": "r2",
                    "command": "show route ipv6 static",
                    "contains": [f"{R1_LOOP_IP}/{HOST_MASK}", r2_peer_ip6],
        ...
        print("BGP loopback static-peer IPv6 check passed.")
    finally:
        _cleanup_case_config(rt, r1_peer_ip6=r1_peer_ip6, r2_peer_ip6=r2_peer_ip6)
