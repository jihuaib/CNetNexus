#!/usr/bin/env python3
"""
BGP negotiation-parameter dual-stack check script.

Goal:
- verify IPv4 and IPv6 transport peers in one module script
- verify router-id change triggers re-negotiation
- verify hold timer change triggers re-negotiation
- verify open AF changes trigger re-negotiation
...
def _session_checks_ipv6(r1_peer_ip6: str, r2_peer_ip6: str) -> list[dict[str, object]]:
    return [
        {
            "device": "r1",
...
def run(rt: TopologyRuntime, top: dict[str, object]) -> None:
    ...
    try:
        ...
        _cleanup(rt)
        _run_inner_ipv6(rt, r1_peer_ip6, r2_peer_ip6)
    finally:
        step("Cleanup BGP config")
        _cleanup(rt)

    print("BGP negotiation parameter dual-stack check passed.")
...
def _run_inner_ipv6(rt: TopologyRuntime, r1_peer_ip6: str, r2_peer_ip6: str) -> None:
    ...
    wait_checks(rt, _session_checks_ipv6(r1_peer_ip6, r2_peer_ip6), timeout=40)
