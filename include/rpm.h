...
/* 策略用途采用 bitmask；业务模块只订阅自己消费的用途。 */
#define RPM_POLICY_TYPE_BGP_EXPORT (1u << 0)
#define RPM_POLICY_TYPE_BGP_IMPORT (1u << 1)
#define RPM_POLICY_TYPE_REDISTRIBUTE (1u << 2)
#define RPM_POLICY_TYPE_ALL 0xFFFFFFFFu
...
/**
 * 订阅策略事件。type_mask 是用途位图，RPM 仅推送有交集的策略。
 */
...
/**
 * 查询指定名称且用途匹配的策略。不存在或类型不匹配时返回 ERRCODE_SUCCESS，
 * 并将 out->found 置 0。
 */
