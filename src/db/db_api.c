...
gboolean db_validate_field(const char *db_name, const char *table_name, const char *field_name,
                              const db_value_t *value, char *error_msg, uint32_t error_msg_len)
{
    ...
    return cli_param_type_validate(field->param_type, value_str, error_msg, error_msg_len);
}
