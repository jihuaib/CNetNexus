...
int ipc_frame_parse_header(const uint8_t *buf, ipc_frame_header_t *header)
{
    if (!buf || !header)
    {
        return ERRCODE_FAIL;
...
ipc_message_t *ipc_frame_to_message(const ipc_frame_header_t *header, const uint8_t *payload)
{
    if (!header)
    {
        return NULL;
    ...
    return ipc_message_create(header->msg_type, header->src_module_id, header->dst_module_id,
                                  header->request_id, data_copy, header->payload_len, g_free);
}
