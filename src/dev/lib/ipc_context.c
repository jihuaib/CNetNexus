...
static void handle_frame(ipc_context_t *ctx, ipc_connection_t *conn, ipc_frame_header_t *header,
...
static void attempt_reconnects(ipc_context_t *ctx)
{
    ...
    for (int i = 0; i < ctx->num_connections; i++)
    {
        ...
        /* 查找目标地址 */
        const ipc_module_addr_t *addr = ipc_config_find(&ctx->config, conn->remote_module_id);
        if (!addr)
        {
            continue;
        }
...
ipc_context_t *ipc_init(uint32_t module_id, const char *name, const char *config_path,
                               ipc_msg_handler_fn msg_handler)
{
    ...
    /* 创建并绑定 IPC 监听端口 */
    const ipc_module_addr_t *my_addr = ipc_config_find(&ctx->config, module_id);
    if (my_addr)
    {
        ctx->listen_fd = create_listen_socket(my_addr->host, my_addr->port);
        if (ctx->listen_fd >= 0)
        {
            struct epoll_event ev;
            ev.events = EPOLLIN;
...
ipc_message_t *ipc_query(ipc_context_t *ctx, uint32_t target_module_id, ipc_message_t *msg,
                                uint32_t timeout_ms)
{
    if (!ctx || !msg)
    {
