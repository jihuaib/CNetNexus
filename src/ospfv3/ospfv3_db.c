/**
 * @file   ospfv3_db.c
 * @brief  OSPFv3 persistent configuration and startup restore
 */
...
#define OSPFV3_VRF_TABLE_INSTANCE "vrf_instance"

static const db_column_def_t OSPFV3_INSTANCE_COLS[] = {
...
static const db_table_def_t OSPFV3_INSTANCE_TABLE = {
    .table_name = OSPFV3_TABLE_INSTANCE,
    .cols = OSPFV3_INSTANCE_COLS,
    .num_cols = G_N_ELEMENTS(OSPFV3_INSTANCE_COLS),
};

static const db_column_def_t OSPFV3_AREA_COLS[] = {
...
static const db_table_def_t OSPFV3_AREA_TABLE = {
    .table_name = OSPFV3_TABLE_AREA,
    .cols = OSPFV3_AREA_COLS,
    .num_cols = G_N_ELEMENTS(OSPFV3_AREA_COLS),
};
...
static const db_table_def_t OSPFV3_INTERFACE_TABLE = {
    .table_name = OSPFV3_TABLE_INTERFACE,
    .cols = OSPFV3_INTERFACE_COLS,
    .num_cols = G_N_ELEMENTS(OSPFV3_INTERFACE_COLS),
};

static void ospfv3_db_instance_pk(db_filter_builder_t *pk, uint32_t process_id)
{
    db_filter_init(pk);
    db_filter_add_int(pk, "process_id", (int64_t)process_id);
}

static void ospfv3_db_area_pk(db_filter_builder_t *pk, uint32_t process_id, uint32_t area_id)
{
    db_filter_init(pk);
    db_filter_add_int(pk, "process_id", (int64_t)process_id);
...
static void ospfv3_db_interface_pk(db_filter_builder_t *pk, uint32_t process_id, const char *ifname)
{
    db_filter_init(pk);
    db_filter_add_int(pk, "process_id", (int64_t)process_id);
...
static gboolean ospfv3_db_int_is_u32(int64_t value)
{
    return value >= 0 && (uint64_t)value <= UINT32_MAX;
}

static void ospfv3_db_if_cfg_defaults(ospfv3_if_cfg_t *cfg, const char *ifname)
{
    if (!cfg)
    {
        return;
...
static void ospfv3_db_if_cfg_sanitize(ospfv3_if_cfg_t *cfg)
{
    if (!cfg)
    {
        return;
    ...
    if (cfg->network_type != OSPFV3_NETWORK_BROADCAST && cfg->network_type != OSPFV3_NETWORK_POINT_TO_POINT)
    {
        cfg->network_type = OSPFV3_NETWORK_BROADCAST;
    }
    if (cfg->cost == 0u)
    ...
    if (cfg->dead_interval == 0u || cfg->dead_interval > UINT16_MAX)
    {
        cfg->dead_interval = OSPFV3_DEFAULT_DEAD_INTERVAL;
    }
}

static void ospfv3_db_if_cfg_from_row(const db_row_t *row, ospfv3_if_cfg_t *cfg)
{
    if (!row || !cfg)
    {
        return;
    ...
    cfg->area_id = ospfv3_db_int_is_u32(area_id) ? (uint32_t)area_id : OSPFV3_AREA_BACKBONE;
    cfg->network_type =
...
static int ospfv3_db_cleanup_orphan_rows(dev_ipc_context_t *ctx, const char *child_table)
{
    ...
    for (uint32_t i = 0u; i < result->num_rows; ++i)
    {
        ...
        int rc = db_rpc_exists(ctx, OSPFV3_TABLE_INSTANCE, &parent_pk.filter, &parent_exists);
        db_filter_clear(&parent_pk);
        if (rc != ERRCODE_SUCCESS)
        {
            LOG_ERROR("OSPFV3: failed to check parent for %s row %" PRId64, child_table, row_id);
            status = ERRCODE_FAIL;
            continue;
        }
        if (parent_exists)
        ...
        if (rows < 0)
        {
            LOG_ERROR("OSPFV3: failed to remove orphan %s row %" PRId64, child_table, row_id);
            status = ERRCODE_FAIL;
            continue;
        }
    ...
    if (removed > 0u)
    {
        LOG_WARN("OSPFV3: removed %u orphan row(s) from %s", removed, child_table);
    }
...
int ospfv3_db_init(void)
{
    ...
    static const db_table_def_t *const tables[] = {
        &OSPFV3_INSTANCE_TABLE,
        &OSPFV3_AREA_TABLE,
        &OSPFV3_INTERFACE_TABLE,
    };
    ...
    for (size_t i = 0u; i < G_N_ELEMENTS(tables); ++i)
    {
        ...
        {
            LOG_ERROR("OSPFV3: create table %s failed", tables[i]->table_name);
            return ERRCODE_FAIL;
        }
    }

    /* Versioned names allow a later cascade definition to coexist during schema upgrades. */
    if (db_rpc_create_delete_cascade(ctx, "ospfv3_instance_delete_interfaces_v1", OSPFV3_TABLE_INSTANCE, "process_id",
                                     OSPFV3_TABLE_INTERFACE, "process_id") != ERRCODE_SUCCESS ||
        db_rpc_create_delete_cascade(ctx, "ospfv3_instance_delete_areas_v1", OSPFV3_TABLE_INSTANCE, "process_id",
                                     OSPFV3_TABLE_AREA, "process_id") != ERRCODE_SUCCESS)
    {
        LOG_ERROR("OSPFV3: create instance delete cascade failed");
        return ERRCODE_FAIL;
    }

    int interface_cleanup_rc = ospfv3_db_cleanup_orphan_rows(ctx, OSPFV3_TABLE_INTERFACE);
    int area_cleanup_rc = ospfv3_db_cleanup_orphan_rows(ctx, OSPFV3_TABLE_AREA);
    if (interface_cleanup_rc != ERRCODE_SUCCESS || area_cleanup_rc != ERRCODE_SUCCESS)
    {
        LOG_ERROR("OSPFV3: orphan configuration cleanup failed");
        return ERRCODE_FAIL;
    }
...
int ospfv3_db_set_instance(uint32_t process_id, uint32_t vrf_id, const char *vrf_name)
{
    ...
    ospfv3_db_instance_pk(&pk, process_id);

    gboolean exists = FALSE;
    int rc = db_rpc_exists(ctx, OSPFV3_TABLE_INSTANCE, &pk.filter, &exists);
    if (rc != ERRCODE_SUCCESS)
    {
        db_filter_clear(&pk);
        return ERRCODE_FAIL;
    ...
    if (exists)
    {
        ...
        int rows = db_rpc_update_cols(ctx, OSPFV3_TABLE_INSTANCE, &pk.filter, cols, G_N_ELEMENTS(cols));
        db_filter_clear(&pk);
    ...
    return db_rpc_insert_cols(ctx, OSPFV3_TABLE_INSTANCE, cols, G_N_ELEMENTS(cols));
}
...
int ospfv3_db_del_instance(uint32_t process_id)
{
    ...
    ospfv3_db_instance_pk(&inst_pk, process_id);
    int inst_rows = db_rpc_delete(ctx, OSPFV3_TABLE_INSTANCE, &inst_pk.filter);
                           ...
                           size_t vrf_name_out_size)
{
    ...
    ospfv3_db_instance_pk(&pk, process_id);
    db_result_t *result = NULL;
    int rc = db_rpc_query(ctx, OSPFV3_TABLE_INSTANCE, NULL, 0, &pk.filter, &result);
    db_filter_clear(&pk);
...
int ospfv3_db_resolve_vrf(const char *vrf_name, uint32_t *vrf_id_out)
{
    ...
    db_result_t *result = NULL;
    int rc = db_rpc_query(ospfv3_local_ipc_ctx(), OSPFV3_VRF_TABLE_INSTANCE, fields, G_N_ELEMENTS(fields),
    ...
    db_result_free(result);
    if (!ospfv3_db_int_is_u32(vrf_id) || vrf_id == VRF_PUBLIC_VRF_ID)
    {
        return ERRCODE_FAIL;
    }
    *vrf_id_out = (uint32_t)vrf_id;
...
int ospfv3_db_area_exists(uint32_t process_id, uint32_t area_id, gboolean *exists)
{
    ...
    *exists = FALSE;
    db_filter_builder_t pk;
    ospfv3_db_area_pk(&pk, process_id, area_id);
    int rc = db_rpc_exists(ctx, OSPFV3_TABLE_AREA, &pk.filter, exists);
    db_filter_clear(&pk);
    return rc;
}
...
{
    dev_ipc_context_t *ctx = ospfv3_local_ipc_ctx();
    if (!ctx || process_id == 0u || ospfv3_db_get_instance(process_id, NULL, NULL, NULL, 0u) != ERRCODE_SUCCESS)
    {
        return ERRCODE_FAIL;
    }
    ...
    return db_rpc_insert_cols(ctx, OSPFV3_TABLE_AREA, cols, G_N_ELEMENTS(cols));
}
...
int ospfv3_db_area_in_use(uint32_t process_id, uint32_t area_id, char *ifname_out, size_t ifname_out_size)
{
    ...
    db_filter_builder_t filter;
    ospfv3_db_area_pk(&filter, process_id, area_id);
    const char *fields[] = {"ifname"};
    db_result_t *result = NULL;
    int rc = db_rpc_query(ctx, OSPFV3_TABLE_INTERFACE, fields, G_N_ELEMENTS(fields), &filter.filter, &result);
    db_filter_clear(&filter);
    if (rc != ERRCODE_SUCCESS)
    {
        if (result)
...
int ospfv3_db_del_area(uint32_t process_id, uint32_t area_id)
{
    ...
    int in_use = ospfv3_db_area_in_use(process_id, area_id, NULL, 0u);
    if (in_use != 0)
    {
        return ERRCODE_FAIL;
    }
    ...
    db_filter_builder_t pk;
    ospfv3_db_area_pk(&pk, process_id, area_id);
    int rows = db_rpc_delete(ctx, OSPFV3_TABLE_AREA, &pk.filter);
    db_filter_clear(&pk);
    return rows >= 0 ? ERRCODE_SUCCESS : ERRCODE_FAIL;
}
...
int ospfv3_db_set_interface(uint32_t process_id, const ospfv3_if_cfg_t *cfg)
{
    ...
    ospfv3_if_cfg_t safe = *cfg;
    safe.ifname[sizeof(safe.ifname) - 1u] = '\0';
    ospfv3_db_if_cfg_sanitize(&safe);

    gboolean area_exists = FALSE;
    if (ospfv3_db_area_exists(process_id, safe.area_id, &area_exists) != ERRCODE_SUCCESS || !area_exists)
    {
        LOG_WARN("OSPFV3: process %u area %u is not configured", process_id, safe.area_id);
        return ERRCODE_FAIL;
    }

    db_filter_builder_t pk;
    ospfv3_db_interface_pk(&pk, process_id, safe.ifname);

    gboolean exists = FALSE;
    int rc = db_rpc_exists(ctx, OSPFV3_TABLE_INTERFACE, &pk.filter, &exists);
    if (rc != ERRCODE_SUCCESS)
    {
        db_filter_clear(&pk);
        return ERRCODE_FAIL;
    ...
    if (exists)
    {
        int rows = db_rpc_update_cols(ctx, OSPFV3_TABLE_INTERFACE, &pk.filter, cfg_cols, G_N_ELEMENTS(cfg_cols));
        db_filter_clear(&pk);
        return rows >= 0 ? ERRCODE_SUCCESS : ERRCODE_FAIL;
    }
    ...
    return db_rpc_insert_cols(ctx, OSPFV3_TABLE_INTERFACE, insert_cols, G_N_ELEMENTS(insert_cols));
}
...
int ospfv3_db_get_interface(uint32_t process_id, const char *ifname, ospfv3_if_cfg_t *cfg_out)
{
    ...
    db_filter_builder_t pk;
    ospfv3_db_interface_pk(&pk, process_id, ifname);
    db_result_t *result = NULL;
    int rc = db_rpc_query(ctx, OSPFV3_TABLE_INTERFACE, NULL, 0, &pk.filter, &result);
    db_filter_clear(&pk);
    if (rc != ERRCODE_SUCCESS || !result || result->num_rows == 0u)
    {
        if (result)
    ...
    ospfv3_db_if_cfg_defaults(cfg_out, ifname);
    ospfv3_db_if_cfg_from_row(result->rows[0], cfg_out);
    ospfv3_db_if_cfg_sanitize(cfg_out);
    db_result_free(result);
    return ERRCODE_SUCCESS;
}
...
int ospfv3_db_del_interface(uint32_t process_id, const char *ifname)
{
    ...
    db_filter_builder_t pk;
    ospfv3_db_interface_pk(&pk, process_id, ifname);
    int rows = db_rpc_delete(ctx, OSPFV3_TABLE_INTERFACE, &pk.filter);
    db_filter_clear(&pk);
    return rows >= 0 ? ERRCODE_SUCCESS : ERRCODE_FAIL;
}

static int ospfv3_db_restore_instances(GHashTable *active_instances)
{
    dev_ipc_context_t *ctx = ospfv3_local_ipc_ctx();
    db_result_t *result = NULL;
    ...
    if (db_rpc_query(ctx, OSPFV3_TABLE_INSTANCE, NULL, 0, NULL, &result) != ERRCODE_SUCCESS)
    {
        return ERRCODE_FAIL;
    }
    if (!result)
    ...
    for (uint32_t i = 0u; i < result->num_rows; ++i)
    {
        ...
        if (process_id_value <= 0 || !ospfv3_db_int_is_u32(process_id_value))
        {
            LOG_WARN("OSPFV3: skipping invalid persisted process id");
            continue;
        }
        if (!admin_up)
        {
        ...
        apply.u.instance_set.router_id = ospfv3_db_int_is_u32(router_id_value) ? (uint32_t)router_id_value : 0u;
        apply.u.instance_set.vrf_id = ospfv3_db_int_is_u32(vrf_id_value) ? (uint32_t)vrf_id_value : VRF_PUBLIC_VRF_ID;
        ...
        if (ospfv3_worker_dispatch_apply(&apply) != ERRCODE_SUCCESS || apply.rc == OSPFV3_APPLY_RC_FAIL)
        {
            LOG_WARN("OSPFV3: failed to restore process %u", process_id);
            status = ERRCODE_FAIL;
            continue;
        }
...
static guint64 ospfv3_db_area_key(uint32_t process_id, uint32_t area_id)
{
    return ((guint64)process_id << 32u) | area_id;
}

static gboolean ospfv3_db_active_area_contains(GHashTable *active_areas, uint32_t process_id, uint32_t area_id)
{
    guint64 key = ospfv3_db_area_key(process_id, area_id);
    return g_hash_table_contains(active_areas, &key);
}

static void ospfv3_db_active_area_add(GHashTable *active_areas, uint32_t process_id, uint32_t area_id)
{
    if (ospfv3_db_active_area_contains(active_areas, process_id, area_id))
    {
        return;
    }

    guint64 *key = g_new(guint64, 1);
    *key = ospfv3_db_area_key(process_id, area_id);
    g_hash_table_add(active_areas, key);
}

static int ospfv3_db_apply_restored_area(uint32_t process_id, uint32_t area_id, GHashTable *active_areas)
{
    if (ospfv3_db_active_area_contains(active_areas, process_id, area_id))
    {
        return ERRCODE_SUCCESS;
    ...
    ospfv3_db_active_area_add(active_areas, process_id, area_id);
    return ERRCODE_SUCCESS;
}

static int ospfv3_db_restore_areas(GHashTable *active_instances, GHashTable *active_areas)
{
    dev_ipc_context_t *ctx = ospfv3_local_ipc_ctx();
    db_result_t *result = NULL;
    ...
    if (db_rpc_query(ctx, OSPFV3_TABLE_AREA, NULL, 0, NULL, &result) != ERRCODE_SUCCESS)
    {
        return ERRCODE_FAIL;
    }
    if (!result)
    ...
    for (uint32_t i = 0u; i < result->num_rows; ++i)
    {
        ...
        if (process_id_value <= 0 || !ospfv3_db_int_is_u32(process_id_value) || !ospfv3_db_int_is_u32(area_id_value))
        {
            LOG_WARN("OSPFV3: skipping invalid persisted area row");
            continue;
        }
        ...
        if (ospfv3_db_apply_restored_area(process_id, area_id, active_areas) != ERRCODE_SUCCESS)
        {
            LOG_WARN("OSPFV3: failed to restore process %u area %u", process_id, area_id);
            status = ERRCODE_FAIL;
        }
    }
...
static int ospfv3_db_restore_interface_area(uint32_t process_id, uint32_t area_id, GHashTable *active_areas)
{
    if (ospfv3_db_active_area_contains(active_areas, process_id, area_id))
    {
        return ERRCODE_SUCCESS;
    ...
    {
        LOG_WARN("OSPFV3: failed to persist legacy process %u area %u", process_id, area_id);
        return ERRCODE_FAIL;
    }
    if (ospfv3_db_apply_restored_area(process_id, area_id, active_areas) != ERRCODE_SUCCESS)
    {
        LOG_WARN("OSPFV3: failed to restore legacy process %u area %u", process_id, area_id);
        return ERRCODE_FAIL;
    }
    return ERRCODE_SUCCESS;
}

static int ospfv3_db_restore_interfaces(GHashTable *active_instances, GHashTable *active_areas)
{
    dev_ipc_context_t *ctx = ospfv3_local_ipc_ctx();
    db_result_t *result = NULL;
    if (!ctx || !active_instances || !active_areas)
    ...
    if (db_rpc_query(ctx, OSPFV3_TABLE_INTERFACE, NULL, 0, NULL, &result) != ERRCODE_SUCCESS)
    {
        return ERRCODE_FAIL;
    }
    if (!result)
    ...
    for (uint32_t i = 0u; i < result->num_rows; ++i)
    {
        ...
        if (process_id_value <= 0 || !ospfv3_db_int_is_u32(process_id_value) || !ifname || ifname[0] == '\0' ||
            strlen(ifname) >= sizeof(((ospfv3_if_cfg_t *)0)->ifname))
        {
            LOG_WARN("OSPFV3: skipping invalid persisted interface row");
            continue;
        }
        ...
        ospfv3_db_if_cfg_defaults(&apply.u.if_set.cfg, ifname);
        ospfv3_db_if_cfg_from_row(row, &apply.u.if_set.cfg);
        ospfv3_db_if_cfg_sanitize(&apply.u.if_set.cfg);

        if (ospfv3_db_restore_interface_area(process_id, apply.u.if_set.cfg.area_id, active_areas) != ERRCODE_SUCCESS)
        {
            status = ERRCODE_FAIL;
            continue;
        }
        ...
        if (ospfv3_worker_dispatch_apply(&apply) != ERRCODE_SUCCESS || apply.rc == OSPFV3_APPLY_RC_FAIL)
        {
            LOG_WARN("OSPFV3: failed to restore process %u interface %s", process_id, ifname);
            status = ERRCODE_FAIL;
        }
    }
...
int ospfv3_db_restore(void)
{
    ...
    int instance_rc = ospfv3_db_restore_instances(active_instances);
    int area_rc = ospfv3_db_restore_areas(active_instances, active_areas);
    int interface_rc = ospfv3_db_restore_interfaces(active_instances, active_areas);
    g_hash_table_destroy(active_areas);
    g_hash_table_destroy(active_instances);
