...
/**
 * @brief 获取注册的所有模板列表
 * @return GList*（元素为 config_template_t*），由调用者负责释放列表（不释放元素）
 */
 ...
 * @brief 模板变量替换结果
 */
typedef struct template_var_value
{
    char *var_name; /**< 变量名称（如 "bgp_protocol.as_number"） */
    char *value;    /**< 替换值 */
} template_var_value_t;
