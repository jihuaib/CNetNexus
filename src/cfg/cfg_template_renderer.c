 ...
 */
static gboolean template_has_data(config_template_t *template)
{
 ...
 * @param template 模板定义
 * @return GHashTable，key=table_name，value=db_result_t*（需要调用者释放结果）
 */
static GHashTable *query_template_databases(config_template_t *template)
{
 ...
 * @param template 模板定义
 * @param query_results 查询结果（GHashTable，key=table_name 或 db_name.table_name，value=db_result_t*）
 * @param row_index 行索引，指定使用第几行数据
 * @return 变量映射表（GHashTable，key=variable_name，value=variable_value_string）
 */
static GHashTable *build_var_map_for_row(config_template_t *template, GHashTable *query_results, uint32_t row_index)
{
    ...
    for (uint32_t i = 0; i < template->body->num_dbs; i++)
    {
        ...
        result = (db_result_t *)g_hash_table_lookup(query_results, lookup_key);
        if (!result)
        {
            result = (db_result_t *)g_hash_table_lookup(query_results, table_name);
        }
        ...
        // 提取指定行的所有字段值
        db_row_t *row = result->rows[row_index];
...
static uint32_t get_max_rows(GHashTable *query_results)
{
    ...
    {
        db_result_t *result = (db_result_t *)value;
...
/**
 * @brief 释放查询结果哈希表（包含所有 db_result_t）
 *
 ...
 * @brief 递归渲染模板及其子模板
 */
static char *render_template_recursive(config_template_t *template, GString *output)
{
    if (!template)
    {
        return NULL;
    ...
    for (uint32_t row_idx = 0; row_idx < max_rows; row_idx++)
    {
        ...
        {
            char *rendered = config_template_render(template, var_map);
            if (rendered)
            {
...
char *cfg_template_renderer_render_all(void)
{
    GString *output = g_string_new("");

    // 获取所有已注册的模板
    GList *templates = config_template_get_all();
    ...
    while (iter)
    {
        config_template_t *template = (config_template_t *)iter->data;
...
char *cfg_template_renderer_render_by_name(const char *template_name)
{
    if (!template_name)
    {
        return NULL;
    ...
    config_template_t *template = config_template_find_by_name(template_name);
    if (!template)
    {
        return NULL;
    }
