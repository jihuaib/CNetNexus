...
cli_param_type_t *cli_param_type_parse(const char *type_str)
{
    ...
    {
        param_type->type = PARAM_TYPE_STRING;
        param_type->validate = param_validate_string;
    ...
    {
        param_type->type = PARAM_TYPE_UINT;
        param_type->validate = param_validate_uint;
    ...
    else if (strcmp(type_name, "int") == 0)
    {
        ...
        param_type->validate = param_validate_int;
    ...
    {
        param_type->type = PARAM_TYPE_IPV4;
        param_type->validate = param_validate_ipv4;
    }
    else if (strcmp(type_name, "ipv6") == 0)
    {
        param_type->type = PARAM_TYPE_IPV6;
        param_type->validate = param_validate_ipv6;
    }
    else if (strcmp(type_name, "ip") == 0)
    {
        param_type->type = PARAM_TYPE_IP;
        param_type->validate = param_validate_ip;
    }
                           ...
                           uint32_t error_msg_size)
{
    ...
    // Try IPv4 first
    if (param_validate_ipv4(param_type, value, NULL, 0))
    {
        return TRUE;
    }

    // Try IPv6
    if (param_validate_ipv6(param_type, value, NULL, 0))
    {
        return TRUE;
    }
