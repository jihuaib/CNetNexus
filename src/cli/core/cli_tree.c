...
cli_tree_node_t *cli_tree_create_node(uint32_t cfg_id, const char *name, const char *description,
...
cli_tree_node_t *cli_tree_find_child(cli_tree_node_t *parent, const char *name)
{
    ...
    for (uint32_t i = 0; i < parent->num_children; i++)
    {
        if ((parent->children[i]->name) &&
            (strncmp(parent->children[i]->name, name, strlen(name)) == ERRCODE_SUCCESS))
        {
...
uint32_t cli_tree_find_children_input_token(cli_tree_node_t *parent, const char *token,
                                               cli_tree_node_t **matches, uint32_t max_matches)
{
...
uint32_t cli_tree_match_command_get_matches(cli_tree_node_t *root, const char *cmd_line,
                                               cli_tree_node_t **matches, uint32_t max_matches)
{
...
void cli_match_result_add_element(cli_match_result_t *result, uint32_t cfg_id, cli_node_type_t type,
