...
cli_element_t *cli_element_create_with_type(uint32_t element_id, uint32_t cfg_id, element_type_t type,
...
void cli_group_add_element(cli_command_group_t *group, cli_element_t *element)
{
    ...
    group->elements =
        (cli_element_t **)realloc(group->elements, (group->num_elements + 1) * sizeof(cli_element_t *));
    if (!group->elements)
    {
        return;
    }
