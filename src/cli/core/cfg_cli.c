...
// ============================================================================
typedef int (*cfg_cli_cmd_group_handler_t)(cfg_tlv_parser_t parser, cfg_cli_out_t *cfg_out,
                                              cfg_cli_resp_out_t *resp_out);

typedef struct cfg_cli_cmd_group_dispatch
{
    uint32_t group_id;
    cfg_cli_cmd_group_handler_t handler;
} cfg_cli_cmd_group_dispatch_t;

int cfg_cli_cmd_group_show(cfg_tlv_parser_t parser, cfg_cli_out_t *cfg_out, cfg_cli_resp_out_t *resp_out);
int cfg_cli_cmd_group_op(cfg_tlv_parser_t parser, cfg_cli_out_t *cfg_out, cfg_cli_resp_out_t *resp_out);

static const cfg_cli_cmd_group_dispatch_t g_cfg_cli_cmd_group_dispatch[] = {
    {CFG_CLI_GROUP_ID_SHOW, cfg_cli_cmd_group_show},
    {CFG_CLI_GROUP_ID_OP, cfg_cli_cmd_group_op},
};

#define CFG_CLI_CMD_GROUP_DISPATCH_COUNT                                                                            \
    (sizeof(g_cfg_cli_cmd_group_dispatch) / sizeof(g_cfg_cli_cmd_group_dispatch[0]))

typedef int (*cfg_cli_cmd_resp_resp_t)(cli_session_t *session, const cfg_cli_out_t *cfg_out,
                                          cfg_cli_resp_out_t *resp_out);

typedef struct cfg_cli_cmd_resp_dispatch
{
    uint32_t group_id;
    cfg_cli_cmd_resp_resp_t handler;
} cfg_cli_cmd_resp_dispatch_t;

int cfg_cli_cmd_group_resp_show(cli_session_t *session, const cfg_cli_out_t *cfg_out,
                                   cfg_cli_resp_out_t *resp_out);
int cfg_cli_cmd_group_resp_op(cli_session_t *session, const cfg_cli_out_t *cfg_out,
                                 cfg_cli_resp_out_t *resp_out);
void cmd_show_cli_history(cli_session_t *session, const cfg_cli_out_t *cfg_out, cfg_cli_resp_out_t *resp_out);

static const cfg_cli_cmd_resp_dispatch_t g_cfg_cli_cmd_resp_dispatch[] = {
    {CFG_CLI_GROUP_ID_SHOW, cfg_cli_cmd_group_resp_show},
    {CFG_CLI_GROUP_ID_OP, cfg_cli_cmd_group_resp_op},
};

#define CFG_CLI_CMD_GROUP_RESP_DISPATCH_COUNT                                                                       \
    (sizeof(g_cfg_cli_cmd_resp_dispatch) / sizeof(g_cfg_cli_cmd_resp_dispatch[0]))

static int cli_cfg_dispatch_by_group_id(uint32_t group_id, cfg_tlv_parser_t parser, cfg_cli_out_t *cfg_out,
                                           cfg_cli_resp_out_t *resp_out)
{
    for (size_t i = 0; i < CFG_CLI_CMD_GROUP_DISPATCH_COUNT; i++)
    {
        if (g_cfg_cli_cmd_group_dispatch[i].group_id == group_id)
        {
            printf("[cfg_cli] Dispatching to group (group_id=%u)\n", group_id);
            return g_cfg_cli_cmd_group_dispatch[i].handler(parser, cfg_out, resp_out);
        }
    }
...
static void cfg_cli_send_response(cli_session_t *session, cfg_cli_out_t *cfg_out,
                                     cfg_cli_resp_out_t *resp_out)
{
    ...
    do
    {
        ...
        for (size_t i = 0; i < CFG_CLI_CMD_GROUP_RESP_DISPATCH_COUNT; i++)
        {
            if (g_cfg_cli_cmd_resp_dispatch[i].group_id == cfg_out->group_id)
            {
                printf("[cfg_cli] Dispatching resp to group (group_id=%u, offset=%u)\n", cfg_out->group_id,
                       resp_out->batch_offset);
                (void)g_cfg_cli_cmd_resp_dispatch[i].handler(session, cfg_out, resp_out);
            }
        }
...
int cfg_cli_handle(cli_match_result_t *result, cli_session_t *session)
{
    ...
    // Pack TLV message
    uint32_t msg_len = 0;
    uint8_t *msg_data = cli_dispatch_pack_tlv(result, &msg_len);
    if (!msg_data)
    {
        return ERRCODE_FAIL;
    }

    cfg_cli_out_t cfg_out;
    cfg_cli_resp_out_t resp_out;
    ...
    CFG_TLV_PARSE_BEGIN(msg_data, msg_len, parser, group_id)
    {
        printf("[cfg_cli] Received CLI command (group_id=%u)\n", group_id);
        cfg_out.group_id = group_id;
        (void)cli_cfg_dispatch_by_group_id(group_id, parser, &cfg_out, &resp_out);
    }
    CFG_TLV_PARSE_END();

    // Send response based on cfg_out and resp_out
    cfg_cli_send_response(session, &cfg_out, &resp_out);
...
int cfg_cli_cmd_group_show(cfg_tlv_parser_t parser, cfg_cli_out_t *cfg_out, cfg_cli_resp_out_t *resp_out)
{
    ...
    CFG_TLV_FOREACH(parser, elem_id, value, len)
    {
        printf("[cfg_cli]   Element ID: %u, Length: %u\n", elem_id, len);

        switch (elem_id)
        {
            case CFG_CLI_SHOW_CFG_ID_COMMON_INFO:
                cfg_out->data.cfg_show.is_common_info = TRUE;
                break;
            case CFG_CLI_SHOW_CFG_ID_HISTORY:
                cfg_out->data.cfg_show.is_history = TRUE;
                break;
            case CFG_CLI_SHOW_CFG_ID_CURRENT_CONFIG:
...
int cfg_cli_cmd_group_op(cfg_tlv_parser_t parser, cfg_cli_out_t *cfg_out, cfg_cli_resp_out_t *resp_out)
{
    ...
    CFG_TLV_FOREACH(parser, elem_id, value, len)
    {
        ...
        switch (elem_id)
        {
            case CFG_CLI_OP_CFG_ID_EXIT:
                cfg_out->data.cfg_op.is_exit = TRUE;
                break;

            case CFG_CLI_OP_CFG_ID_CONFIG:
                cfg_out->data.cfg_op.is_config = TRUE;
                break;
            case CFG_CLI_OP_CFG_ID_END:
...
static void cfg_cli_chunk_output(GString *full, cfg_cli_resp_out_t *resp_out)
{
    ...
    size_t chunk = remaining < (CFG_CLI_MAX_RESP_LEN - 1) ? remaining : (CFG_CLI_MAX_RESP_LEN - 1);
                                   ...
                                   cfg_cli_resp_out_t *resp_out)
{
    ...
    if (cfg_out->data.cfg_show.is_common_info)
    {
        ...
        if (resp_out->batch_offset == 0)
        {
            ...
            {
                print_view_commands_flat(g_cli_engine_ctx->view_tree.root, g_cfg_show_cache);
            }
...
void cmd_show_cli_history(cli_session_t *session, const cfg_cli_out_t *cfg_out, cfg_cli_resp_out_t *resp_out)
{
    ...
    if (resp_out->batch_offset == 0)
    {
        ...
        for (uint32_t i = 0; i < g_cli_engine_ctx->global_history.count; i++)
        {
            const cli_history_entry_t *entry = cli_global_history_get_entry(
                &g_cli_engine_ctx->global_history, g_cli_engine_ctx->global_history.count - 1 - i);
                                 ...
                                 cfg_cli_resp_out_t *resp_out)
{
    ...
    {
        cli_view_node_t *config_view =
            cli_view_find_by_id(g_cli_engine_ctx ? g_cli_engine_ctx->view_tree.root : NULL, CFG_CLI_VIEW_CONFIG);
        if (config_view)
        {
    ...
    {
        cli_view_node_t *config_view = cli_view_find_by_id(g_cli_engine_ctx ? g_cli_engine_ctx->view_tree.root : NULL, CFG_CLI_VIEW_USER);
        if (config_view)
        {
            session->current_view = config_view;
    ...
    if (cfg_out->data.cfg_op.is_exit == TRUE)
    {
        ...
        {
            cli_view_node_t *config_view =
                cli_view_find_by_id(g_cli_engine_ctx ? g_cli_engine_ctx->view_tree.root : NULL, parent_view->view_id);
            if (config_view)
            {
                session->current_view = config_view;
