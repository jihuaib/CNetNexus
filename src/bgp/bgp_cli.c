 ...
 */
static int handle_bgp_protocol_db(ipc_message_t *msg, cfg_db_payload_parser_t *parser)
{
    ...
    char resp_msg_buf[CFG_CLI_MAX_RESP_LEN];
    resp_msg_buf[0] = '\0';
    ...
    {
        // 构建请求：view_id (4字节)
        uint32_t view_id_be = htonl(CFG_CLI_VIEW_BGP);
        ipc_message_t *req = ipc_message_create(
            CFG_MSG_TYPE_GET_VIEW_PROMPT,
            DEV_MODULE_ID_BGP,
            DEV_MODULE_ID_CFG,
            msg->request_id,
            g_memdup(&view_id_be, sizeof(view_id_be)),
            sizeof(view_id_be),
            g_free
        );
    ...
    char out_prompt[CFG_CLI_MAX_PROMPT_LEN];
    snprintf(out_prompt, CFG_CLI_MAX_PROMPT_LEN, view_name, as_number);
    ...
    memcpy(msg_out, out_prompt, CFG_CLI_MAX_PROMPT_LEN);
 ...
 */
static int handle_bgp_peer_show_db(ipc_message_t *msg, cfg_db_payload_parser_t *parser)
{
    ...
    for (uint32_t i = 0; i < result->num_rows; i++)
    {
        ...
        for (uint32_t j = 0; j < row->num_fields; j++)
        {
            ...
            if (bgp_insert_show_row((int)i + 1, row->field_names[j], value_str) != ERRCODE_SUCCESS)
            {
                ...
                char *resp_data = g_strdup("BGP Error: Failed to write show output.\r\n");
                ipc_message_t *resp =
                    ipc_message_create(CFG_MSG_TYPE_CLI_RESP, DEV_MODULE_ID_BGP, msg->src_module_id,
                                          msg->request_id, resp_data, strlen(resp_data) + 1, g_free);
                if (resp)
                {
                    ipc_send_response(g_bgp_local->ipc_ctx, resp);
 ...
 */
static int bgp_dispatch_db_payload(ipc_message_t *msg, cfg_db_payload_parser_t *parser)
{
