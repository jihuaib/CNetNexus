 ...
 * @brief  BGP UPDATE packet encoder for IPv4 labeled-unicast (SAFI=4)
 */
#include <arpa/inet.h>
#include <string.h>
...
static int labeled_nlri_valid(const bgp_nlri_entry_t *nlri)
{
    if (!nlri || nlri->type != BGP_NLRI_PREFIX || nlri->afi != BGP_AFI_IPV4 || nlri->safi != BGP_SAFI_LABELED ||
...
{
    if (!buf || !labeled_nlri_valid(nlri))
    {
        return -1;
    }
