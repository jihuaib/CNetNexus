/**
 * @file   bgp_pkt_build_ipv4qp.c
 * @brief  BGP UPDATE 报文 IPv4 QP（AFI=1, SAFI=253）AF 编码器
...
static int encode_qp_nlri(uint8_t *buf, int buf_size, const bgp_nlri_entry_t *nlri)
{
    ...
    if (pfx_bytes > 0)
    {
        memcpy(buf + pos, &nlri->qp.prefix.addr.u.v4, pfx_bytes);
        pos += pfx_bytes;
    }
    return pos;
}
...
static int ipv4qp_encode_reach_pa(uint8_t *buf, int buf_size, const bgp_nlri_entry_t *nlri,
                                  const bgp_nexthop_t *nexthop)
{
    return encode_mp_reach(buf, buf_size, nlri, nexthop);
}

static int ipv4qp_encode_reach_nlri(uint8_t *buf, int buf_size, const bgp_nlri_entry_t *nlri,
                                    const bgp_nexthop_t *nexthop)
{
    (void)buf;
    (void)buf_size;
...
static int ipv4qp_encode_unreach_wd(uint8_t *buf, int buf_size, const bgp_nlri_entry_t *nlri, const bgp_conn_t *conn)
{
    (void)buf;
    (void)buf_size;
    (void)nlri;
...
static int ipv4qp_encode_unreach_pa(uint8_t *buf, int buf_size, const bgp_nlri_entry_t *nlri, const bgp_conn_t *conn)
{
    (void)conn;
...
static int ipv4qp_encode_reach_pa_packed(uint8_t *buf, int buf_size, const bgp_nlri_entry_t *const *nlri_list,
...
static int ipv4qp_encode_reach_nlri_packed(uint8_t *buf, int buf_size, const bgp_nlri_entry_t *const *nlri_list,
                                           int nlri_count, const bgp_nexthop_t *nexthop, int *out_packed)
{
    (void)buf;
    (void)buf_size;
...
static int ipv4qp_encode_unreach_wd_packed(uint8_t *buf, int buf_size, const bgp_nlri_entry_t *const *nlri_list,
...
static int ipv4qp_encode_unreach_pa_packed(uint8_t *buf, int buf_size, const bgp_nlri_entry_t *const *nlri_list,
                                           int nlri_count, const bgp_conn_t *conn, int *out_packed)
{
    (void)conn;
    if (out_packed)
...
static const bgp_pkt_af_enc_t g_ipv4qp_enc = {
    .afi = BGP_AFI_IPV4,
    .safi = BGP_SAFI_QP,
    .encode_reach_pa = ipv4qp_encode_reach_pa,
    .encode_reach_nlri = ipv4qp_encode_reach_nlri,
    .encode_unreach_wd = ipv4qp_encode_unreach_wd,
    .encode_unreach_pa = ipv4qp_encode_unreach_pa,
    .encode_reach_pa_packed = ipv4qp_encode_reach_pa_packed,
    .encode_reach_nlri_packed = ipv4qp_encode_reach_nlri_packed,
    .encode_unreach_wd_packed = ipv4qp_encode_unreach_wd_packed,
    .encode_unreach_pa_packed = ipv4qp_encode_unreach_pa_packed,
};

void bgp_pkt_build_ipv4qp_register(void)
{
    bgp_pkt_af_enc_register(&g_ipv4qp_enc);
}
