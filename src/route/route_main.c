...
void route_ipc_msg_handler(dev_ipc_context_t *ctx, dev_ipc_message_t *msg)
{
    ...
    switch (msg->msg_type)
    {
        ...
        case ROUTE_MSG_TYPE_NHOBJ_ACQUIRE:
            if (route_worker_post(ROUTE_WORKER_CMD_NHOBJ_ACQUIRE, msg) != 0)
            {
                LOG_WARN("Route: failed to post NHOBJ_ACQUIRE to worker");
                if (msg->request_id != 0)
                {
                    route_msg_ack_t *ack = (route_msg_ack_t *)g_malloc0(sizeof(route_msg_ack_t));
                    if (ack)
        ...
        case ROUTE_MSG_TYPE_NHOBJ_RELEASE:
            if (route_worker_post(ROUTE_WORKER_CMD_NHOBJ_RELEASE, msg) != 0)
            {
                LOG_WARN("Route: failed to post NHOBJ_RELEASE to worker");
                dev_ipc_message_free(msg);
            }
            return;
